"""
Compact Ensemble Diagnostic

Experiment 017

This script evaluates compact classifiers and small ensemble models using
the official Experiment 014 feature set.

Purpose:

    Check whether the performance gains observed with stronger classifiers
    can be approximated by smaller models that may be more realistic for
    future embedded deployment on the ESP32-S3.

This experiment is still offline/diagnostic. It does not implement the
embedded classifier yet.
"""
...
try:
    ...
    raise ImportError(
        "scikit-learn is required for experiment 017. "
...
EXPERIMENT_ID = "017"
EXPERIMENT_FOLDER_NAME = "017_compact_ensemble_diagnostic"
...
def predict_custom_decision_tree(train_dataset, test_dataset, max_depth):
...
def get_compact_sklearn_classifiers():
    classifiers = []

    for depth in [6, 8, 10, 12]:
        classifiers.append(
            {
                "name": f"sklearn_tree_depth{depth}",
    ...
    for n_estimators in [3, 5, 10, 20]:
        classifiers.append(
            {
                "name": f"random_forest_{n_estimators}",
    ...
    for n_estimators in [3, 5, 10, 20]:
        classifiers.append(
            {
                "name": f"extra_trees_{n_estimators}",
    ...
    for n_estimators in [5, 10, 20, 30]:
        classifiers.append(
            {
                "name": f"gradient_boosting_{n_estimators}",
    ...
    for n_estimators in [5, 10, 20]:
        classifiers.append(
            {
                "name": f"gradient_boosting_depth3_{n_estimators}",
                "model": GradientBoostingClassifier(
                    n_estimators=n_estimators,
                    learning_rate=0.10,
                    max_depth=3,
...
def get_experiment_paths():
    ...
    return {
        ...
        "comparison_results": tables_dir / "compact_ensemble_results.csv",
...
def update_experiment_index(best_result):
    ...
    new_row = {
        ...
        "description": "Compact ensemble diagnostic for embedded feasibility",
        ...
        "main_observation": (
            "Best compact model selected by macro F1 and model size"
        ),
    }
...
def save_experiment_notes(
    ...
    lines.append("Experiment 017 - Compact ensemble diagnostic")
    lines.append("")
    lines.append("Objective:")
    lines.append(
        "Evaluate whether compact ensemble models can approach the "
        "performance of stronger classifiers while remaining more plausible "
        "for future embedded implementation."
    )
    lines.append("")
    ...
    lines.append(
        "If a compact ensemble approaches the performance of larger models, "
        "it may become a candidate for embedded approximation. If compact "
        "models remain close to the single-tree baseline, then the gains from "
        "Experiment 016 likely depend on larger model capacity."
    )
...
def estimate_model_units(classifier_name, model):
    if classifier_name.startswith("custom_tree"):
        return 1

    if classifier_name.startswith("sklearn_tree"):
        return 1
...
def get_model_max_depth(classifier_name, model):
    if classifier_name.startswith("custom_tree_depth"):
        return int(classifier_name.replace("custom_tree_depth", ""))
...
def evaluate_predictions(
    ...
    result = {
        ...
        "top_k": top_k,
        "estimated_model_units": estimated_model_units,
        "max_depth": max_depth,
...
def run_experiment():
    ...
    if features_per_sample != EXPECTED_FEATURES_PER_SAMPLE:
        raise ValueError(
            ...
            "Restore the 11-feature pipeline before running experiment 017."
        )
    ...
    sklearn_classifiers = get_compact_sklearn_classifiers()
    ...
    for top_k in valid_top_k_values:
        ...
        for depth in [6, 8, 10, 12]:
            classifier_name = f"custom_tree_depth{depth}"
            ...
            result, artifacts = evaluate_predictions(
                classifier_name,
                top_k,
                train_dataset,
                test_dataset,
            ...
            artifacts_by_key[(classifier_name, top_k)] = artifacts

            print(
                f"{classifier_name:<30} "
                f"accuracy={result['accuracy']:.6f} "
                f"macro_f1={result['macro_f1']:.6f} "
                f"movement_f1={result['movement_f1']:.6f}"
            )
        ...
        for classifier in sklearn_classifiers:
            ...
            estimated_model_units = estimate_model_units(
                classifier_name,
                model,
            )

            max_depth = get_model_max_depth(
                classifier_name,
                model,
            )
    ...
    save_confusion_matrix(
        artifacts_by_key[best_key]["matrix"],
        CLASS_ORDER,
        paths["best_confusion_matrix"],
    )

    save_class_metrics(
        artifacts_by_key[best_key]["per_class_metrics"],
        paths["best_class_metrics"],
    )

    save_predictions(
        artifacts_by_key[best_key]["predictions"],
    ...
    print()
    print("=" * 70)
    print("Compact ensemble diagnostic finished.")
    print("=" * 70)
    print("Best result:")
    print(f"classifier: {best_result['classifier']}")
    print(f"top_k: {best_result['top_k']}")
    print(f"estimated_model_units: {best_result['estimated_model_units']}")
...
def main():
    print()
    print("Compact Ensemble Diagnostic")
    print("=" * 70)
